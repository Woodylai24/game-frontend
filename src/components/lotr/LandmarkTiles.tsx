    ...
    <div className="bg-gray-800 rounded-lg p-3">
      ...
      <div className="flex gap-2 flex-wrap">
        {faceUpTiles.map(tile => (
          <button key={tile.id} onClick={() => isMyTurn && onTakeLandmark(tile.id)}
            ...
            <img src={getLandmarkImagePath(tile.id)} alt={tile.name}
              className={`w-16 h-20 sm:w-20 sm:h-24 rounded border-2 object-cover
        ...
        {Array.from({ length: faceDownCount }).map((_, i) => (
          <div key={`fd-${i}`} className="w-16 h-20 sm:w-20 sm:h-24 rounded bg-gray-700 border-2 border-gray-600 flex items-center justify-center">
