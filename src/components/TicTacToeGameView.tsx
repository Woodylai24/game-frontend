        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center space-x-4">
              <span className="text-sm text-gray-400">Playing as: {username}</span>
              <button onClick={backToRoom} className="text-sm bg-gray-700 px-3 py-1.5 rounded-md hover:bg-gray-600">Back to Room</button>
            </div>
          </div>
        </div>
      </header>
          ...
          {isFinished && (
            <div className="text-center mb-6">
              <div className="text-2xl font-bold">
                {isDraw ? <span className="text-yellow-400">Game Over — Draw!</span> : <span className="text-green-400">Game Over — {session.winnerUsername} wins!</span>}
              </div>
            </div>
          )}
          <TicTacToeBoard />
